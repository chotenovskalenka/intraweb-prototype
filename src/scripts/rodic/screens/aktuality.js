...
function renderAktuality(){
  ...
  if(ostatni.length){
    h+=`<div class="tile newsbox">`+ostatni.map(n=>`<button class="newsrow" onclick="openNovinka('${n.id}')"><span class="newsdate">${n.date}</span><span class="newsline">${n.t}</span><span class="newsarr">›</span></button>`).join('')+`</div>`;
  }
