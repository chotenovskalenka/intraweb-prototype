...
function renderDrawer(){
  ...
  d.innerHTML=`<div class="dh">Vhaaji<small>průvodcovská appka</small></div>`+SECTIONS.map(s=>`<button class="ditem ${section===s[0]?'on':''}" onclick="go('${s[0]}')"><span class="ic">${s[2]}</span>${s[1]}</button>`).join('');
}
function render(){
