...
function renderKalendar(){
  ...
  if(evs.length)h+=evs.map(a=>`<div class="np" style="cursor:pointer" onclick="openAkceDetail('${a.id}')"><span>${a.name} <span style="color:var(--hint)">›</span></span><b style="font-weight:500;color:var(--ochre)">${a.time||(a.place||'akce')}</b></div>`).join('');
  ...
  if(!isWE(kalSel))h+=`<div class="np" style="border-top:1px solid var(--line);margin-top:4px;padding-top:8px"><span>Slouží</span><b style="font-weight:500">${gws.map(g=>g.n).join(', ')||'—'}</b></div>`;
  h+=`</div>`;
  return h;
}
