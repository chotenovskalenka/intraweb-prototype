...
let FONDHIST=[];
let fondFormOpen=false, odecet=null;
...
function renderFond(){
  ...
  let h=`<div class="tile note-info">Fond je <b>per dítě</b>. Po akci se strhne jen dětem, které ten den <b>přišly</b> a <b>šly na akci</b> — ne celé třídě. Spravuje hospodářka.</div>`;
  h+=`<div class="vhead">Placené akce</div>`;
  h+= paidAkce.length? paidAkce.map(a=>`<div class="frow"><span class="adate">${dayLbl(a)}</span><div style="flex:1"><div class="aname">${a.name}</div><div class="ameta">${a.paid} Kč/dítě</div></div>${a.done?'<span class="bdg pre" style="padding:6px 10px">odečteno ✓</span>':`<button class="odbtn" onclick="startOdecet('${a.id}')">Odečíst</button>`}</div>`).join('') : `<div class="empty">Žádné placené akce. Cenu nastavíš u akce v Plán &amp; program.</div>`;
  ...
  h+=`<div class="rosterbox">`+data.map((c,i)=>({c,i})).sort(byAlpha).map(({c})=>`<div class="row"><div class="rmain" style="cursor:default">${avatar(c,26)}<span class="nm">${full(c)}</span><span class="ind" style="color:${c.fond<300?'var(--danger)':'var(--forest)'};font-weight:600">${c.fond.toLocaleString('cs-CZ')} Kč</span></div></div>`).join('')+`</div>`;
...
function renderOdecet(){
  ...
  h+=`<div class="tile"><div class="tlab">Odečet po akci</div><div style="font-family:var(--serif);font-size:17px;color:var(--forest-dk)">${a.name}</div><div class="ameta">${dayLbl(a)} · ${a.paid} Kč/dítě</div></div>`;
  ...
  h+=`<div class="rosterbox">`+(present.length?present.filter(x=>!odQuery||norm(full(x.c)).includes(norm(odQuery))).sort(byAlpha).map(x=>`<div class="row"><div class="rmain" onclick="togOdChild(${x.i})"><span class="chk ${odecet.sel.has(x.i)?'on':''}">${odecet.sel.has(x.i)?'✓':''}</span>${avatar(x.c,26)}<span class="nm">${full(x.c)}</span>${x.c.predskolak?'<span class="bdg pre">předšk.</span>':''}<span class="ind" style="color:var(--muted)">${x.c.fond.toLocaleString('cs-CZ')} Kč</span></div></div>`).join('') : '<div class="empty">Ten den nikdo nepřišel.</div>')+`</div>`;
