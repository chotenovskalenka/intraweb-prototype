...
function temaLoniBtn(){
  const o=temaObdobi();
  if(o.stav==='archiv'){
    // opačný směr: z archivu založit tentýž měsíc příští rok
    const cil=temaSousedniRok(o,1);
    return cil?`<button class="btn-ghost" onclick="temaAdopt('${o.key}','${cil.key}')">Použít pro ${cil.label.toLowerCase()}</button>`:'';
  }
...
function temaBlock(){
  ...
  // archiv je jen ke čtení – jeho jediná akce („Použít pro…") sedí nahoře u nadpisu
  if(!ro)h+=`<button class="btn-primary btn-block tema-save" onclick="showToast('Tématický plán uložen ✓')">Uložit ${o.stav==='bezi'?'tématický plán':'plán na '+o.label.toLowerCase()}</button>`;
  return h;
}
